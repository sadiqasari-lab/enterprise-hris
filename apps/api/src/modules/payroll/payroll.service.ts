...
export class PayrollService {
  ...
  async submitForReview(cycleId: string, submittedBy: string): Promise<any> {
    ...
    await notificationService.notifyRole(cycle.company_id, ['HR_ADMIN'], {
      title: 'Payroll ready for review',
      message: `Payroll cycle ${cycleId} submitted for review.`,
      type: 'PAYROLL',
      channel: 'BOTH',
    });
  ...
  async reviewPayroll(
    ...
    if (approved) {
      ...
      await notificationService.notifyRole(cycle.company_id, ['GM'], {
        title: 'Payroll awaiting GM approval',
        message: `Payroll cycle ${cycleId} approved by HR Admin. GM approval required.`,
        type: 'PAYROLL',
        channel: 'BOTH',
      });
      ...
      await notificationService.notifyRole(cycle.company_id, ['HR_OFFICER'], {
        title: 'Payroll rejected',
        message: `Payroll cycle ${cycleId} rejected by HR Admin. Please correct and resubmit.`,
        type: 'PAYROLL',
        channel: 'BOTH',
      });
  ...
  async gmApproval(
    ...
    if (approved) {
      ...
      await notificationService.notifyRole(cycle.company_id, ['HR_ADMIN'], {
        title: 'Payroll approved by GM',
        message: `Payroll cycle ${cycleId} approved by GM. Ready for execution.`,
        type: 'PAYROLL',
        channel: 'BOTH',
      });
      ...
      // TODO: Send notification to HR Admin
      await notificationService.notifyRole(cycle.company_id, ['HR_ADMIN'], {
        title: 'Payroll rejected by GM',
        message: `Payroll cycle ${cycleId} rejected by GM. Please review and resubmit.`,
        type: 'PAYROLL',
        channel: 'BOTH',
      });
  ...
  async executePayroll(cycleId: string, executedBy: string): Promise<any> {
    ...
    const employeeIds = updatedCycle.records.map((record) => record.employee_id);
    await notificationService.notifyEmployeesByEmployeeIds(updatedCycle.company_id, employeeIds, {
      title: 'Payslip available',
      message: `Your payslip for cycle ${cycleId} is now available.`,
      type: 'PAYROLL',
      channel: 'BOTH',
    });
